# Next Chat Handoff

## Project Goal

QuantPilot-AI 2.0 is an open-source-first, adapter-first, contract-first, A-share-first AI quant integration platform designed to evolve toward a profitable AI quantitative trading platform through evidence-gated research.

## Current Phase

Phase 7D: external analytics preflight, implemented by Codex and pending ChatGPT closure review.

## Completed Work

Step 0A through Phase 7C are completed.

Phase 7D created:

- `src/quantpilot_core/factors/external_analytics_preflight.py`
- `data/factor_validation/external_analytics_preflight.json`
- `docs/EXTERNAL_ANALYTICS_PREFLIGHT.md`
- `tests/factors/test_external_analytics_preflight.py`
- Phase 7D module kickoff and closure draft docs

No external analytics package was installed or imported.

## Current Prohibitions

- do not fetch market data
- do not call external APIs
- do not install packages
- do not add Alphalens, quantstats, empyrical, Qlib, pandas, NumPy, or other external analytics packages
- do not run factor tear sheets or portfolio reports
- do not claim alpha, profitability, or statistical significance
- do not run real backtests
- do not implement strategy tournament
- do not implement production adapters
- do not implement broker/live/order or agent workflows
- do not mark anything trading-ready

## Next Recommended Step

ChatGPT should perform Phase 7D closure review.

Do not move to external package installation, larger real-data validation, strategy tournament, or real alpha claims until approved.

## Key Decisions

- Phase 7D is preflight only.
- External analytics are candidates, not evidence.
- No candidate is approved for install or adapter work.
- Python standard library is sufficient for this module.
- Codex is not the project architect.

## Role Split

ChatGPT is the architecture lead and strategic reviewer.

Codex creates files, organizes docs, runs safe validation commands, and produces review packets.

ChatGPT, not Codex, performs module closure review.
