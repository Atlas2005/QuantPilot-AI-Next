...
The first QMT bridge bootstrap does not require a snapshot that the not-yet-started exporter cannot have written. After configuration and the account-binding key are safely persisted, bootstrap runs Runtime Doctor with the explicit provisioning-only flag `--allow-missing-qmt-snapshot-during-provisioning`. That narrow mode accepts only an existing bridge directory with a genuinely missing completed snapshot, and it revalidates the account-binding key without exposing its material. A temp-only write, malformed, stale, or unsafe snapshot, invalid key, identity mismatch, or any other bridge failure still fails bootstrap. Repeated bootstrap before the first export is therefore idempotent and allowed.
...
5. Confirm that the first completed snapshot appears after QMT invokes `handlebar(ContextInfo)`, followed by bounded periodic snapshots. `after_init` schedules the first query cycle without querying the broker, and QMT `run_time` timer callbacks only mark later cycles as due.

The exporter invokes the QMT built-in `get_trade_detail_data` system function directly by its injected global name because the broker runtime injects that function into the QMT strategy namespace. This QMT runtime requires an active strategy-bar context, so all broker read-only system queries run only from `handlebar(ContextInfo)`; `init`, `after_init`, and timer callbacks do not query the broker. A due handlebar cycle queries `account`, `position`, `order`, and `deal` in that order. This is a QMT built-in Python strategy, not an external XtQuant connection. It never calls an order-submission or cancellation function. A failed section query is recorded in the heartbeat instead of stopping the other read-only queries.
...
The provisioning exception is bootstrap-only. `status_windows_runtime_v1.ps1`, `inspect_qmt_builtin_bridge_v1.ps1`, and `inspect_qmt_builtin_bridge_v1.py` never pass the provisioning flag: before the first completed export they remain strictly `NOT_READY`. `provider=none` is unchanged and needs no provisioning exception. The intended first-run order is bootstrap, create/start the built-in QMT exporter, then run status or inspection after `latest_snapshot_v1.json` appears.
